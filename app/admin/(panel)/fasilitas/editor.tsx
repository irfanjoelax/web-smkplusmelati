...
export default function FasilitasEditor({ initial }: { initial: FasilitasItem[] }) {
  const [items, setItems] = useState<FasilitasItem[]>(() => tagPersistedItems(initial));
  const { save } = useManualSave("fasilitas", items);
  const [confirmIdx, setConfirmIdx] = useState<number | null>(null);
  const [search, setSearch] = useState("");
  const [editingIdx, setEditingIdx] = useState<number | null>(null);
  const [draft, setDraft] = useState<FasilitasItem | null>(null);
  ...
  async function remove(i: number) {
    await deleteEditorItem("fasilitas", null, items[i], () => {
  ...
  if (draft) {
    return (
      <div>
        <PageHeader
          title={editingIdx === null ? "Tambah Fasilitas" : "Edit Fasilitas"}
          description={editingIdx === null ? "Tambahkan data fasilitas baru." : draft.title || "Perbarui data fasilitas."}
        />
        <Panel
          title={editingIdx === null ? "Fasilitas Baru" : draft.title || "Edit Fasilitas"}
          description="Pilih foto, lalu isi nama dan deskripsi fasilitas."
          action={
  ...
  const filteredItems = items
    ...
    .filter(({ item }) =>
      !query || `${item.title} ${item.description}`.toLocaleLowerCase("id-ID").includes(query),
    );
  ...
  return (
    <div>
      ...
      <Panel
        title="Fasilitas"
        description={`${items.length} fasilitas terdaftar`}
        action={
          <div className="flex items-center gap-2">
            <label htmlFor="fasilitas-page-size" className="sr-only">Jumlah fasilitas per halaman</label>
            <select
              id="fasilitas-page-size"
              value={pageSize}
              onChange={(event) => {
      ...
      >
        <div className="mb-5">
          <label htmlFor="search-fasilitas" className="sr-only">Cari fasilitas</label>
        ...
        {filteredItems.length === 0 ? (
          <p className="rounded-xl bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">
            {items.length === 0 ? "Belum ada fasilitas. Klik Tambah untuk mulai." : "Fasilitas tidak ditemukan."}
          </p>
        ) : (
            ...
            {visibleItems.map(({ item, index }) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 line-clamp-2 text-sm text-slate-500">{item.description || "Deskripsi belum diisi"}</p>
                </div>
                <div className="flex shrink-0 gap-1">
                  <IconBtn label={`Edit ${item.title || "fasilitas"}`} onClick={() => edit(index)}>
                    <EditIcon className="h-4 w-4" />
                  </IconBtn>
                  <IconBtn label={`Hapus ${item.title || "fasilitas"}`} danger onClick={() => setConfirmIdx(index)}>
